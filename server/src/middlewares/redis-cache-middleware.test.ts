...
import {
  ...
  invalidateCache,
  redisCache,
} from "@/middlewares/redis-cache-middleware";

type MockRedisClient = {
  del: ReturnType<
    typeof mock<(key: string, ...keys: string[]) => Promise<number>>
  >;
  exists: ReturnType<typeof mock<(key: string) => Promise<boolean>>>;
  expire: ReturnType<
    typeof mock<(key: string, ttl: number) => Promise<number>>
  >;
  get: ReturnType<typeof mock<(key: string) => Promise<string | null>>>;
  send: ReturnType<
    typeof mock<(command: string, args: string[]) => Promise<unknown>>
  >;
