...
type RedisCommandClient = Pick<
  typeof redis,
  "del" | "exists" | "expire" | "get" | "send" | "set" | "ttl"
>;
  ...
  async delByPattern(pattern: string, namespace: string): Promise<number> {
    try {
      ...
      const keys = Array.isArray(keysResult)
        ? keysResult.filter((key): key is string => typeof key === "string")
        : [];
...
export const redisCache = (options: RedisCacheOptions = {}) => {
  ...
  return createMiddleware<AppBindings>(
    async (c: Context<AppBindings>, next: Next) => {
      ...
      const cacheKey =
        typeof key === "function" ? key(c) : key || generateCacheKey(c);
...
export const userCache = (
  keyPattern: string,
  options: Omit<RedisCacheOptions, "key"> = {},
) => {
  return redisCache({
    ...options,
    key: (c) => {
