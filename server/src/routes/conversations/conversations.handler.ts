...
export const createConversation: AppRouteHandler<
  CreateConversationRoute
> = async (c) => {
...
export const getConversations: AppRouteHandler<GetConversationsRoute> = async (
  c,
) => {
  const userJwt = c.get("jwtPayload").sub;
...
export const getConversation: AppRouteHandler<GetConversationRoute> = async (
  c,
) => {
  const userJwt = c.get("jwtPayload").sub;
...
export const updateConversation: AppRouteHandler<
  UpdateConversationRoute
> = async (c) => {
  const userJwt = c.get("jwtPayload").sub;
  const { id } = c.req.param();
