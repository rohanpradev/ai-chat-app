...
import type { CacheEntry } from "@/middlewares/redis-cache-middleware";
...
const mockRedisCache = {
  ...
  get: mock<() => Promise<CacheEntry | null>>(() => Promise.resolve(null)),
...
describe("Profile Routes with Redis Cache", () => {
  ...
  describe("GET /profile/me", () => {
    ...
    it("should handle cache hit scenario", async () => {
      ...
      mockRedisCache.get.mockResolvedValueOnce(cachedData as CacheEntry);
  ...
  describe("Cache Operations", () => {
    ...
    it("should test cache invalidation", async () => {
      const { invalidateCache } = await import(
        "@/middlewares/redis-cache-middleware"
      );
