...
export const createEmbeddingDocument = async ({
	...
	const usageRequestId = await reserveUsage({
		...
		estimatedTokens: chunks.reduce((total, chunk) => total + chunk.tokenEstimate, 0),
		model: EMBEDDING_MODEL_ID,
		resourceBytes: byteSize,
		scope: "embedding",
		userId
	});
	...
	try {
		...
		if (firstEmbedding.length !== EMBEDDING_DIMENSIONS) {
			throw new Error(`Embedding provider returned ${firstEmbedding.length} dimensions; expected ${EMBEDDING_DIMENSIONS}`);
		}
		...
		await settleUsage(usageRequestId, {
			inputTokens: embeddingResult.usage?.tokens,
...
export const searchEmbeddings = async (userId: string, request: EmbeddingSearchRequest, abortSignal?: AbortSignal) => {
	const usageRequestId = await reserveUsage({
		...
		estimatedTokens: estimateUsageTokens(request.query),
		model: EMBEDDING_MODEL_ID,
		scope: "embedding",
		userId
	});
	try {
		...
		if (queryEmbedding.embedding.length !== EMBEDDING_DIMENSIONS) {
			throw new Error(
				`Embedding provider returned ${queryEmbedding.embedding.length} dimensions; expected ${EMBEDDING_DIMENSIONS}`
			);
		}
		const similarity = sql<number>`1 - (${cosineDistance(embeddingChunks.embedding, queryEmbedding.embedding)})`;
		const rows = await db
			.select({
			...
			.where(and(...filters, gte(similarity, request.minScore)))
		...
		await settleUsage(usageRequestId, {
			inputTokens: queryEmbedding.usage?.tokens,
			totalTokens: queryEmbedding.usage?.tokens
		});
		return result;
	} catch (error) {
		await settleUsage(usageRequestId, { status: "failed" });
...
export const answerWithRag = async (userId: string, request: RagRequest, abortSignal?: AbortSignal) => {
	...
	const prompt = buildRagPrompt(request.query, search.results);
	const usageRequestId = await reserveUsage({
		category: "rag-answer",
		estimatedTokens: estimateUsageTokens(prompt) + 2048,
		model: selectedModel.id,
		scope: "ai",
		userId
	});
